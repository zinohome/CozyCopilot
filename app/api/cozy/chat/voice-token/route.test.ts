...
function makeReq(body: unknown, auth: string | null = BEARER): NextRequest {
  ...
  return new Request("http://localhost/api/cozy/chat/voice-token", {
    method: "POST",
    headers,
...
describe("POST /api/cozy/chat/voice-token", () => {
  ...
  it("returns 400 with details.session_id when session_id is not a uuid", async () => {
    const res = await POST(
      makeReq({ session_id: "not-a-uuid", personality_id: PERSONALITY_ID }),
    );
    expect(res.status).toBe(400);
    const body = await res.json();
    expect(body.error.code).toBe("VALIDATION_ERROR");
  ...
  it("returns 400 with details.personality_id when personality_id is not a uuid", async () => {
    const res = await POST(
      makeReq({ session_id: SESSION_ID, personality_id: "not-a-uuid" }),
    );
    expect(res.status).toBe(400);
    const body = await res.json();
    expect(body.error.code).toBe("VALIDATION_ERROR");
  ...
  it("forwards to CozyEngineV2 and returns envelope on 200", async () => {
    installFetchMock(200, { token: "livekit-token", url: "wss://livekit", room: "room-1" });
    ...
    expect(body).toEqual({
      ok: true,
      data: { token: "livekit-token", url: "wss://livekit", room: "room-1" },
    });
  });
  ...
  it("trusts upstream body code (LIVEKIT_FAILED → 500)", async () => {
    installFetchMock(500, { code: "LIVEKIT_FAILED", message: "join failed" });

    const res = await POST(makeReq(validBody));
    expect(res.status).toBe(500);
    const body = await res.json();
    expect(body.error.code).toBe("LIVEKIT_FAILED");
  });
});
