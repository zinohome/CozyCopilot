// BFF fetch helper for the sessions feature.
//
// Mirrors src/features/providers/apiFetch.ts. Kept in the feature directory
// because the shared src/lib/api/client.ts is GET-only; session CRUD needs
// POST/PATCH/DELETE. Returns the BFF envelope's `data` field on success and
// throws an `ApiError` (built from the envelope's `error` block) on failure.
